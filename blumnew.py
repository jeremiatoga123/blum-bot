...
run_config = {
    'game_params': {
        'min_clover': 200,
        'max_clover': 200,
        'trump': lambda: random.randint(0, 10),
        'harris': lambda: random.randint(0, 10),
        'freeze': 0
    }
}
...
def generate_payload(game_id, clover_amount, trump, harris, freeze, dogs_eligible=False, max_retries=3, delay=2):
    global should_exit
    payload_data = {
        "gameId": game_id,
        "clover": clover_amount,
        "trump": trump,
        "harris": harris,
        "freeze": freeze
    }
...
def get_game_parameters():
    ...
    print(f"Note: CLOVER points will be selected randomly between min and max values")
    print(f"Maximum is 280 (some account got 324)\n{RESET}")
    print(f"{CYAN}{'='*52}{RESET}")
    try:
        min_input = input(f"{GREEN}Enter minimum CLOVER points : {RESET}")
        max_input = input(f"{GREEN}Enter maximum CLOVER points : {RESET}")
        min_clover = int(min_input) if min_input else 200
        max_clover = int(max_input) if max_input else 200
        if min_clover > max_clover:
            min_clover, max_clover = max_clover, min_clover
            
        print(f"\n{YELLOW}Additional Assets Settings (0 if not using):{RESET}")
        trump_input = input(f"{GREEN}Enter TRUMP clicks      : {RESET}")
        harris_input = input(f"{GREEN}Enter HARRIS clicks     : {RESET}")
        freeze_input = input(f"{GREEN}Enter FREEZE clicks     : {RESET}")
        
        trump = int(trump_input) if trump_input else lambda: random.randint(0, 10)
        harris = int(harris_input) if harris_input else lambda: random.randint(0, 10)
        freeze = int(freeze_input) if freeze_input else 0

        
        return {
            'min_clover': min_clover,
            'max_clover': max_clover,
            'trump': trump,
            'harris': harris,
            'freeze': freeze
        }
    except ValueError:
        print(f"{RED}Input must be a number! Using defaults{RESET}")
        return {
            'min_clover': 200,
            'max_clover': 200,
            'trump': lambda: random.randint(0, 10),
    ...
    try: 
        ...
        while not should_exit:
            try:
                ...
                if play_passes <= 0:
                    print(f"[{username}] : {YELLOW}No more Ticket available. Total profit: {account_profit}, Games played: {account_games_played}{RESET}")
                    return
                ...
                clover_amount = random.randint(
                    run_config['game_params']['min_clover'],
                    run_config['game_params']['max_clover']
                )

                trump_clicks = run_config['game_params']['trump']() if callable(run_config['game_params']['trump']) else run_config['game_params']['trump']
                harris_clicks = run_config['game_params']['harris']() if callable(run_config['game_params']['harris']) else run_config['game_params']['harris']
                freeze_clicks = run_config['game_params']['freeze']

                result = generate_payload(
                    game_id, 
                    clover_amount,
                    trump=trump_clicks,    
                    harris=harris_clicks, 
                    freeze=freeze_clicks,  
                    dogs_eligible=dogs_eligible
                )
                ...
                payload, dogs = result  
                print(f"[{username}] : {CYAN}Using clover amount : {RESET}{GREEN}{clover_amount}{RESET}")
                print(f"[{username}] : {CYAN}Using TRUMP clicks  : {RESET}{GREEN}{trump_clicks}{RESET}")
                print(f"[{username}] : {CYAN}Using HARRIS clicks : {RESET}{GREEN}{harris_clicks}{RESET}")
                print(f"[{username}] : {CYAN}Using FREEZE clicks : {RESET}{GREEN}{freeze_clicks}{RESET}")
                if dogs_eligible:
                    print(f"[{username}] : {CYAN}Using dogs amount : {RESET}{GREEN}{dogs}{RESET}")
        ...
        print(f"{RED}Fatal error in play process: {str(e)}{RESET}")
        return None
...
def auto_loop_process(queries):
    global should_exit, farming_summary, session
    ...
    run_config['game_params'] = game_params
    print(f"\n{CYAN}Game Parameters Summary:{RESET}")
    print(f"{GREEN}CLOVER Range    : {game_params['min_clover']} - {game_params['max_clover']}{RESET}")
    if callable(game_params['trump']):
        print(f"{GREEN}TRUMP Clicks    : Random (0-10)")
    else:
        print(f"{GREEN}TRUMP Clicks    : {game_params['trump']}{RESET}")
    if callable(game_params['harris']):
        print(f"{GREEN}HARRIS Clicks   : Random (0-10){RESET}")
    else:
        print(f"{GREEN}HARRIS Clicks   : {game_params['harris']}{RESET}")
    print(f"{GREEN}FREEZE Clicks   : {game_params['freeze']}{RESET}")
    
    try:
        num_threads = int(input(f"\n{CYAN}Enter the number of threads to use : {RESET}"))
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=num_threads) as executor:
                ...
                try:
                    ...
                    print(f"{RED}Error in thread execution: {str(e)}{RESET}")
                    continue  
                finally:
                    for future in future_to_query.keys():
                        future.cancel()
            ...
            print(f"{RED}Error in auto loop: {str(e)}{RESET}")
            time.sleep(5)  
            continue
        ...
        while not should_exit:
            try:
                ...
                if choice == '2':
                    ...
                    if callable(game_params['trump']):
                        print(f"{GREEN}TRUMP Clicks    : Random (0-10){RESET}")
                    else:
                        print(f"{GREEN}TRUMP Clicks    : {game_params['trump']}{RESET}")
                ...
                print(f"{RED}Error in main menu: {str(e)}{RESET}")
                time.sleep(2)
                continue

    except KeyboardInterrupt:
        print(f"{YELLOW}Program terminated by user{RESET}")
    except Exception as e:
